 ...
 *
 * Deployments:
 *   localhost (dev)      → backend on :3502 (Vite proxy forwards /api in dev)
 *   flagia.devmeko.xyz   → https://flagiaapi.devmeko.xyz  (tunneled, local :3502)
 *   flagia.kr            → https://api.flagia.kr           (tunneled, local :3000)
 *   other host / local IP → same host on :3502
 *
 ...
 * only matters for the localhost / raw-IP fallback (3502).
 */

const LOCAL_PORT = 3502
...
export function resolveWsUrl(): string {
  if (import.meta.env.DEV) return `ws://localhost:${LOCAL_PORT}/ws`
  const host = window.location.hostname
  const apiHost = API_HOST_MAP[host]
