...
function scoreCv(cv) {
    ...
    if (cv >= 0.3 && cv <= 0.8)
        return 100; // Natural human range — perfect
...
function getCvDescription(cv) {
    if (cv >= 0.3 && cv <= 0.8)
        return '자연스러운 사람의 타이핑 리듬이 관찰됩니다. 키 입력 간격의 변동이 사람의 일반적인 범위 내에 있습니다.';
    if (cv >= 0.2 && cv < 0.3)
        return '타이핑 리듬이 약간 균일하지만, 대부분의 자연스러운 타이핑 패턴과 일치합니다.';
    if (cv > 0.8 && cv <= 1.0)
        return '타이핑 리듬에 다소 높은 변동이 관찰됩니다. 간헐적인 사고나 수정 작업이 포함된 것으로 보입니다.';
    if (cv >= 0.1 && cv < 0.2)
        return '타이핑 리듬이 상당히 균일합니다. 미리 작성된 텍스트를 옮겨 쓰고 있을 가능성이 있습니다.';
    if (cv > 1.0 && cv <= 1.5)
        return '키 입력 간격의 변동이 매우 큽니다. 외부 소스에서 간헐적으로 복사했을 가능성이 있습니다.';
    if (cv < 0.1)
        return '키 입력이 기계적으로 균일합니다. 자동 입력 도구나 매크로 사용이 의심됩니다.';
 ...
 */
function generateVerdict(flagiaScore, flagStatus, cv, rr, pasteCount, blurSeconds, mode) {
    let verdict;
    let details = [];
    if (flagStatus === 'GREEN') {
        ...
        if (scoreCv(cv) < 70)
            issues.push('타이핑 리듬');
        if (scoreRevisionRatio(rr) < 70)
            issues.push('수정 패턴');
        if (scorePasteCount(pasteCount) < 70)
            issues.push('외부 콘텐츠');
        if (scoreBlurDuration(blurSeconds) < 70)
            issues.push('화면 이탈');
        if (issues.length > 0)
 ...
 */
function runFlagiaAnalysis(events, finalMarkdown, templateText, mode) {
    ...
    // ── Verdict ──
    const { verdict, verdictDetail } = generateVerdict(flagiaScore, flagStatus, cv, revisionRatio, totalPasteCount, totalBlurDuration, mode);
    return {
        flagiaScore,
        flagStatus,
