...
async function register(role, label, stamp) {
...
async function requestForceSubmit(inbox, assignmentId, prefix) {
    const requestId = `${prefix}-${Date.now()}-${Math.random()}`;
    inbox.ws.send(JSON.stringify({ type: 'force_submit', payload: { assignmentId, requestId } }));
    return inbox.waitFor(message => (message.type === 'force_submit_ack' || message.type === 'force_submit_error') &&
        message.payload?.requestId === requestId, 15_000);
}
async function run() {
    ...
    try {
        ...
        const alreadyBefore = before.find((item) => item.id === alreadySub.id);
        ...
        const onlineAfter = after.find((item) => item.id === onlineSub.id);
        const offlineAfter = after.find((item) => item.id === offlineSub.id);
        const alreadyAfter = after.find((item) => item.id === alreadySub.id);
...
//# sourceMappingURL=force_submit_test.js.map