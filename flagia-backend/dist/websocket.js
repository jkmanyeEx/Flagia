    ...
    wss.on('connection', (ws, req) => {
        ...
        ws.on('message', async (raw) => {
            try {
               ...
               VALUES (?, ?, NOW(), ?, ?, '[]')`, [sessionId, submissionId, ip, userAgent]);
        ...
        ws.on('close', async () => {
            ...
            if (closedSessionId) {
                ...
                await database_1.default.query('UPDATE sessions SET end_time = NOW() WHERE id = ? AND end_time IS NULL', [closedSessionId]).catch(() => { });
            }
            // Remove from rooms
