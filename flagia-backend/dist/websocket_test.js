...
async function runTest() {
    ...
    const teacherWs = new ws_1.default(WS_URL);
    let teacherMessages = [];
    ...
    const studentWs = new ws_1.default(WS_URL);
    let studentMessages = [];
    ...
    await delay(1500);
    let foundSubmissionsUpdate = teacherMessages.some(m => m.type === 'submissions_update' && m.payload.assignmentId === assignment.id);
    ...
    await delay(1500);
    foundSubmissionsUpdate = teacherMessages.some(m => m.type === 'submissions_update' && m.payload.assignmentId === assignment.id);
    if (foundSubmissionsUpdate) {
...
//# sourceMappingURL=websocket_test.js.map