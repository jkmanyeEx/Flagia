...
function runReplay(events, templateText) {
  // Convert HTML template into plain text, replacing paragraph blocks with newlines to match ProseMirror structure.
...
async function main() {
  ...
  for (const sessId of sessionIds) {
    ...
    const reconstructed = runReplay(events, template_text || '');
    
    // Replace block tags with newline to properly compare plain text
