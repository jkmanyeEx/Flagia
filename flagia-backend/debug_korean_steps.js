...
const isJung = (k) => JUNG_LIST.includes(k);
...
async function main() {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (e.type === 'keydown') {
      ...
      actionStr = `keydown key='${key}' rawPos=${cursorPosition} mappedPos=${pos}`;
      ...
      if (key === 'Backspace') {
        if (e.timestamp - lastBackspaceTime < 15) {
          if (i >= 100 && i <= 220) {
            console.log(`Step ${i}: ${actionStr} => [SKIPPED DUPLICATE BACKSPACE]`);
          }
