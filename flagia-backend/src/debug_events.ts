...
async function debug() {
  ...
  const [rows] = await pool.query(
    `SELECT s.id, s.flagia_score, s.flag_status, s.analysis_json
     FROM submissions s WHERE s.id IN (?, ?)`,
    ['7a3281c2-0924-4046-81dd-923e7bf359a2', 'ce65d9dc-a7de-45dc-8292-e3c27f5622ef']
  );
  ...
  const [sess1] = await pool.query(
    'SELECT events_blob FROM sessions WHERE submission_id = ? ORDER BY start_time ASC',
    ['7a3281c2-0924-4046-81dd-923e7bf359a2']
  );
  for (const s of sess1 as any[]) {
    if (s.events_blob) {
      const events = JSON.parse(s.events_blob);
      console.log(`  Total events: ${events.length}`);
      events.slice(0, 15).forEach((e: any, i: number) => {
  ...
  const [sess2] = await pool.query(
    'SELECT events_blob FROM sessions WHERE submission_id = ? ORDER BY start_time ASC',
    ['ce65d9dc-a7de-45dc-8292-e3c27f5622ef']
  );
  for (const s of sess2 as any[]) {
    if (s.events_blob) {
      const events = JSON.parse(s.events_blob);
      console.log(`  Total events: ${events.length}`);
      // Find Korean keydown events
      const koreanEvents = events.filter((e: any) => 
        e.type === 'keydown' && e.meta?.key && /[\uac00-\ud7af\u3131-\u3163\u314f-\u3163]/.test(e.meta.key)
      );
      console.log(`  Korean keydown events: ${koreanEvents.length}`);
      koreanEvents.slice(0, 30).forEach((e: any, i: number) => {
