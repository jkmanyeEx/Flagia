...
router.post('/join/:code', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    await pool.query(
      ...
      [submissionId, assignmentId, user.userId, '', 'IN_PROGRESS']
    );
...
router.post('/', authMiddleware, teacherOnly, async (req: Request, res: Response) => {
  try {
    ...
    await pool.query(
      `INSERT INTO assignments (id, teacher_id, title, due_date, time_limit, text_limit, max_score, template_text, mode, join_code)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, user.userId, title, new Date(dueDate), timeLimit || 60, textLimit || 3000, maxScore || 100, templateText || '', mode || 'STANDARD', joinCode]
    );
